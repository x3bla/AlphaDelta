...
class AutoPlay:
    ...
    @staticmethod
    async def manually_delete(file_name):
        os.remove(file_name)
...
class Music(commands.Cog):
    ...
    async def play(self, ctx, *, title=None):
        ...
        if title.lower() == "queue" or title.lower() == "q":
            ...
            #
            # async def deletefrfr():
            #     await asyncio.sleep(10)
            #     os.remove("youtube-YjM3C5ukqzk.m4a")
            #
            # def delete(error):
            #     deletefrfr()

            with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                secret = 'https://www.youtube.com/watch?v=YjM3C5ukqzk'
                info = await temp(secret)
                file = ydl.prepare_filename(info)

                source = discord.FFmpegOpusAudio(file)
                ctx.message.guild.voice_client.play(source, after=lambda error : os.remove("youtube-YjM3C5ukqzk.m4a"))
                return
